 ...
 * We exchange the code for a session and redirect to /portal/change-password
 * so the user can set their password.
 */
export async function GET(request: NextRequest) {
  const { searchParams } = new URL(request.url)
  const code = searchParams.get("code")
  const next = "/portal/change-password"

  if (!code) {
    return NextResponse.redirect(
      new URL("/portal/login?error=link_invalido", request.url)
    )
  }
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      new URL("/portal/login?error=link_expirado", request.url)
    )
  }
