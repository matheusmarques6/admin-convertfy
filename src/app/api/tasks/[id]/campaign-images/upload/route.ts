/**
 * POST /api/tasks/[id]/campaign-images/upload
 *
 * Upload da imagem-base (referência visual) de um lote de imagens de campanha.
 * Grava no bucket onboarding-visual-assets e devolve a URL assinada (365d) +
 * path. Espelha o padrão de /api/admin/stores/[id]/brand-identity/upload.
 *
...
const log = logger.child("CampaignImageUpload")
...
export async function POST(
  ...
  try {
    ...
    const path = `campaigns/${source_id}/image-refs/${ts}-${safeFilename}`
    ...
    return errorResponse(request, error, "CampaignImageUpload")
  }
}
