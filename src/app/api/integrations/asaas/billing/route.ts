...
export async function GET(request: NextRequest) {
  try {
    ...
    const period = searchParams.get("period") || "month" // today, 7days, month, year, custom
    ...
    // Get customer count and subscriptions
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Erro ao buscar faturamento" },
      { status: 500 }
    )
  }
}
