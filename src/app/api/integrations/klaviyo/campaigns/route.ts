...
export async function GET(request: NextRequest) {
  try {
    ...
    // Read-first: check structured cache (klaviyo_campaign_metrics)
    ...
    if (!forceRefresh) {
      ...
      if (cachedMetrics && cachedMetrics.length > 0) {
        log.info(`[Cache HIT] Returning ${cachedMetrics.length} campaigns from structured cache`)

        let campaignsWithMetrics = cachedMetrics.map(row => ({
          id: row.campaign_id,
        ...
        const totals = campaignsWithMetrics.reduce((acc, campaign) => ({
          ...
          totalCampaigns: 0, sentCampaigns: 0, scheduledCampaigns: 0, draftCampaigns: 0,
          cancelledCampaigns: 0, emailCampaigns: 0, smsCampaigns: 0, totalRecipients: 0,
          totalDelivered: 0, totalOpened: 0, totalClicked: 0, totalConversions: 0,
          totalRevenue: 0, totalBounced: 0, totalUnsubscribed: 0, totalSpamComplaints: 0
        })
