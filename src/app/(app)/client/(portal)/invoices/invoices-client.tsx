...
interface InvoicesClientProps {
  initialData: InvoicesResponse | null
  initialStatus: InvoiceStatusData | null
}

export function InvoicesClient({ initialData, initialStatus }: InvoicesClientProps) {
  const [invoices, setInvoices] = useState<Invoice[]>(initialData?.invoices ?? [])
  const [nextInvoice, setNextInvoice] = useState<Invoice | null>(initialData?.nextInvoice ?? null)
  const [stats, setStats] = useState<InvoicesResponse["stats"] | null>(initialData?.stats ?? null)
  ...
  const [revenueGenerated, setRevenueGenerated] = useState<number | null>(
    typeof initialStatus?.revenueGenerated === "number" ? initialStatus.revenueGenerated : null
  )
  const [showRevenue, setShowRevenue] = useState(!!initialStatus?.showRevenue)
  ...
  // O servidor já entregou os dados iniciais; só busca no mount se não vieram.
