...
export interface TrackingInitialData {
  stores: TrackingStoreData[]
}

interface TrackingClientProps {
  initialData: TrackingInitialData | null
  initialStoreId: string | null
}

export function TrackingClient({ initialData, initialStoreId }: TrackingClientProps) {
  const origin = useOrigin()
  const initialStore = initialData && initialData.stores.length > 0 ? initialData.stores[0] : null
  const initialSavedColor = initialStore?.widget_config?.primaryColor || "#3b82f6"
  // Overview state
  const [store, setStore] = useState<TrackingStoreData | null>(initialStore)
  ...
  // Customize state
  const [color, setColor] = useState(initialSavedColor)
  const [customColor, setCustomColor] = useState(
    initialStore?.widget_config && !PRESET_COLORS.some(c => c.value === initialSavedColor)
      ? initialSavedColor
      : ""
  )
  const [language, setLanguage] = useState(initialStore?.widget_config?.language || "pt-BR")
