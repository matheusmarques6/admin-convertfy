import { cookies } from "next/headers"
import { redirect } from "next/navigation"
import { createAdminClient } from "@/lib/supabase/admin"
import { getPortalUser } from "@/lib/services/portal-auth.service"
import { buildPortalDashboard } from "@/lib/services/portal-dashboard.service"
import { getPortalOnboardingStatus } from "@/lib/services/portal-onboarding-status.service"
import { OnboardingDashboard } from "./onboarding-dashboard"
import { DashboardClient } from "./dashboard-client"
import type { DashboardData } from "./types"

export default async function PortalDashboardPage() {
  const portalUser = await getPortalUser()
  if (!portalUser) {
    redirect("/client/login")
  }

  const adminClient = createAdminClient()

  // Gate: onboarding ativo mostra o dashboard de onboarding no lugar
  const { status: onboardingStatus } = await getPortalOnboardingStatus(
    portalUser.client_id,
    adminClient
  )
  if (onboardingStatus && onboardingStatus !== "completed") {
    return <OnboardingDashboard firstName={portalUser.name?.split(" ")[0] || ""} />
  }

  const cookieStore = await cookies()
  const storeId = cookieStore.get("portal_active_store")?.value || null

  let initialData: DashboardData | null = null
  try {
    initialData = (await buildPortalDashboard({
      clientId: portalUser.client_id,
      portalUserEmail: portalUser.email,
      period: "30d",
      storeId,
      adminClient,
    })) as unknown as DashboardData
  } catch {
    // Sem dados iniciais o client component busca via API (comportamento antigo)
    initialData = null
  }

  return <DashboardClient initialData={initialData} initialStoreId={storeId} />
}
