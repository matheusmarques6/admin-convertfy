 ...
 *
 * Builds the full portal dashboard payload (client, stores, invoices,
 * campaigns, meetings, Klaviyo + Shopify cache-first data) for a client.
 *
 * Extracted from portal/dashboard/route.ts so Server Components can call
...
// ─── buildPortalDashboard: full dashboard payload (cache-first) ──────────────

export async function buildPortalDashboard(opts: {
  clientId: string
  portalUserEmail?: string
  period: string
  storeId: string | null
  ...
  const { clientId, portalUserEmail, period, storeId, adminClient } = opts
  ...
  const response: Record<string, unknown> = {
    ...
    meetings: await resolvePortalMeetings(meetings, adminClient, portalUserEmail),
