...
export function normalizarOutputDetalhado(parsed: unknown): OutputNormalizado {
  ...
  const estrutura: EstruturadorPosicao[] = cru
    ...
    .map((p, i) => {
      ...
      // Dispositivo de OUTRA seção ("pergunta_ao_leitor" numa posição body) é
...
- "imagem": a cena que a foto desta posição precisa mostrar (ex.: "uso real em corpo adulto, não estúdio"). É a instrução de maior peso do agente de imagem. OBRIGATÓRIA onde a seção/forma tem foto gerada (<secoes_disponiveis> diz "com imagem gerada: N"): sem ela a posição sai com a mesma foto do hero. Uma frase concreta — quem, fazendo o quê, onde — e cada posição é OUTRO momento da história: nunca repita a cena de outra posição (o hero mostra a promessa; a tese mostra o mecanismo; a garantia mostra o uso). Só pode ficar null onde nenhuma variante tem foto.
- <secoes_disponiveis> diz, por seção, o que a biblioteca TEM (quantas variantes, faixa de itens, quantas mostram preço/avaliação/cupom/CTA, quantas têm imagem gerada). Não exija o que não existe: se exigir, a posição pode ficar sem candidata — prefira ajustar o papel e registrar em "exige" o que faltou.
...
- Exemplo de requisitos preenchidos num toque COM incentivo: hero → {"dispositivo":"codigo_entregue","cupom":true,"cta":true,"n_itens":null,"preco":null,"avaliacao":null,"campos":[],"imagem":"uso real em corpo adulto, luz natural","exige":["código visível","sem prazo"]}; products → {"dispositivo":"vitrine_paralela","cupom":false,"cta":true,"n_itens":{"min":2,"max":3},"preco":true,"avaliacao":null,"campos":["preco","nome"],"imagem":null,"exige":["preço em texto real"]}.
