/**
 * Embeddings da base de conhecimento — OpenAI text-embedding-3-small
 * (1536 dims, a coluna `embedding vector(1536)`). Sem OPENAI_API_KEY
 * devolve null e a busca degrada para full-text (a sync e o chat
...
export async function embedTexts(texts: string[]): Promise<number[][] | null> {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`OpenAI embeddings ${resp.status}: ${snippet}`)
    }
