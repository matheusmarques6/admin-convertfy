...
const capaDaNeon = () => formatosParaOTipo("molde-post", "capa").find((s) => s.templateId === "molde-neon")!
...
describe("importarSlide", () => {
  it("entra antes do CTA final e não depois dele", () => {
    ...
    const novo = importarSlide(d, capaDaNeon())
    expect(novo.frames).toHaveLength(d.frames.length + 1)
    expect(novo.frames[novo.frames.length - 1].tipo).toBe("cta")
    expect(novo.frames[novo.frames.length - 2].label).toBe(capaDaNeon().label)
  })
  ...
  it("o slide entra na identidade do DESTINO, não na do molde de origem", () => {
    ...
    const novo = importarSlide(d, capaDaNeon())
    const importado = novo.frames[novo.frames.length - 2]
    // A Neon é preta; o fundo aqui tem de ser o da família do documento.
    expect(novo.fundoPorFrame[importado.frameId]).not.toBe(FAMILIAS.neon.fundoEscuro)
...
describe("aplicarSlideNoFrame", () => {
  it("troca o formato e PRESERVA a copy do operador", () => {
    ...
    const novo = aplicarSlideNoFrame(d, 0, capaDaNeon())
    expect(novo.frames[0].textos.titulo).toBe("O que ninguém te conta")
    expect(novo.frames[0].label).toBe(capaDaNeon().label)
  })
  ...
  it("a variação volta ao padrão: ela endereçava o desenho do molde anterior", () => {
    ...
    expect(aplicarSlideNoFrame(d, 0, capaDaNeon()).frames[0].variante).toBeUndefined()
  })
  ...
  it("a imagem FICA quando o formato novo não tem slot — apagá-la perderia o upload", () => {
    ...
    const semFoto = { ...capaDaNeon(), slotsImagem: 0 as const }
  ...
  it("índice fora da lista devolve o MESMO documento", () => {
    const d = doc()
    expect(aplicarSlideNoFrame(d, 99, capaDaNeon())).toBe(d)
  })

  it("o molde do documento não muda: um slide emprestado não reescreve a origem da peça", () => {
    const d = doc()
    expect(aplicarSlideNoFrame(d, 0, capaDaNeon()).templateId).toBe("molde-post")
    expect(importarSlide(d, capaDaNeon()).templateId).toBe("molde-post")
  })
})
