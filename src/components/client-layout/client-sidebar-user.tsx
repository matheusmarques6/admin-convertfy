...
interface ClientSidebarUserProps {
  user?: {
    name: string
    email: string
    clientName: string
...
export function ClientSidebarUser({ user, collapsed = false }: ClientSidebarUserProps) {
          ...
          <>
            <div className="text-left overflow-hidden flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-700 dark:text-[#EAEDF3] truncate leading-tight">
                {user?.clientName || user?.name || "Usuario"}
              </p>
  ...
  const dropdownContent = (
    ...
    >
      ...
      <DropdownMenuItem asChild className="rounded-md mx-1 px-2">
        <Link href={ROUTES.CLIENT.CHANGE_PASSWORD}>
          <Icon icon={Lock} size={16} className="mr-2 text-muted-foreground" />
          Alterar Senha
        </Link>
      </DropdownMenuItem>
      <DropdownMenuSeparator />
