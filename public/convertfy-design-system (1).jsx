...
function AreaChart({ data, prev, labels, color = DS.colors.brand.blueLight, height = 220, title, sub }) {
...
function SecDashboard() {
  const kpis=[
    {l:"Receita atribuída",v:"R$ 2.4M",d:"+22.1%",u:true,sp:[1.1,1.3,1.5,1.6,1.8,1.9,2.0,2.1,2.2,2.3,2.4]},
    {l:"Emails enviados",v:"1.82M",d:"+18.3%",u:true,sp:[1.0,1.15,1.2,1.3,1.4,1.5,1.55,1.6,1.7,1.75,1.82]},
    {l:"Taxa de conversão",v:"4.7%",d:"+0.3%",u:true,sp:[3.8,4.0,4.1,4.1,4.2,4.3,4.3,4.4,4.5,4.6,4.7]},
  ];
  ...
  const sm={Ativo:{bg:DS.colors.sem.posBg,t:DS.colors.sem.posText,b:DS.colors.sem.posBorder},Pausado:{bg:DS.colors.sem.warnBg,t:DS.colors.sem.warnText,b:DS.colors.sem.warnBorder},Rascunho:{bg:DS.colors.sem.neutBg,t:DS.colors.sem.neutText,b:DS.colors.sem.neutBorder}};
  const meetings=[
    {t:"Review AchaTudo — Q1 Results",time:"14:00",dur:"30min"},
    {t:"Onboarding Donaris Joias",time:"16:00",dur:"45min"},
    {t:"Briefing Clube Rock — Páscoa",time:"10:00 amanhã",dur:"30min"},
  ];
  const tasks=[
    {t:"Criar flow Dia das Mães — EPORTH",p:"Alta",st:"Pendente"},
    {t:"Audit deliverability — Based 3.0",p:"Alta",st:"Em progresso"},
    {t:"Template novo — Donaris Joias",p:"Média",st:"Pendente"},
    {t:"Report mensal — Oak Vintage",p:"Baixa",st:"Pendente"},
  ];
  const pMap={Alta:{bg:DS.colors.sem.negBg,t:DS.colors.sem.negText,b:DS.colors.sem.negBorder},Média:{bg:DS.colors.sem.warnBg,t:DS.colors.sem.warnText,b:DS.colors.sem.warnBorder},Baixa:{bg:DS.colors.sem.neutBg,t:DS.colors.sem.neutText,b:DS.colors.sem.neutBorder}};
  ...
  return <div style={{background:DS.colors.gray[50],minHeight:"100vh"}}>
    ...
    <div style={{background:DS.colors.gray.white,borderBottom:`1px solid ${DS.colors.border}`,padding:`${DS.s[4]}px ${DS.s[8]}px`}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"baseline",gap:DS.s[3]}}>
          <span style={{fontSize:22,fontWeight:600,color:DS.colors.gray[900],fontFamily:DS.f.sans,letterSpacing:"-0.02em"}}>Dashboard</span>
    ...
    <div style={{padding:DS.s[6],maxWidth:1280,margin:"0 auto"}}>
      ...
      {/* ROW 1: Highlight Card + 3 KPI Cards */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:DS.s[4],marginBottom:DS.s[6]}}>
        ...
        <div style={{
          ...
          <div>
            <div style={{fontSize:13,fontWeight:500,opacity:0.75,fontFamily:DS.f.sans,marginBottom:DS.s[2]}}>Receita total recuperada</div>
            <div style={{fontSize:32,fontWeight:600,fontFamily:DS.f.sans,letterSpacing:"-0.03em",fontVariantNumeric:"tabular-nums lining-nums",lineHeight:1}}>R$ 1.24M</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6,marginTop:DS.s[4]}}>
            <span style={{background:"rgba(255,255,255,0.15)",borderRadius:DS.r.pill,padding:"2px 8px",fontSize:11,fontWeight:600,letterSpacing:"0.02em"}}>↑ +34.2%</span>
            <span style={{fontSize:11,opacity:0.6}}>vs período anterior</span>
          </div>
        </div>

        {/* 3 KPI Cards normais */}
        {kpis.map((s,i)=><div key={i} style={{background:DS.colors.gray.white,borderRadius:DS.r.md,padding:`${DS.s[4]}px ${DS.s[6]}px`,border:`1px solid ${DS.colors.border}`,display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
          <div>
            <div style={{fontSize:13,fontWeight:500,color:DS.colors.gray[500],fontFamily:DS.f.sans,marginBottom:DS.s[3]}}>{s.l}</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
              <div style={{fontSize:32,fontWeight:600,color:DS.colors.gray[900],fontFamily:DS.f.sans,letterSpacing:"-0.03em",fontVariantNumeric:"tabular-nums lining-nums",lineHeight:1}}>{s.v}</div>
              <Spark data={s.sp} color={s.u?DS.colors.sem.posText:DS.colors.brand.blueLight} w={64} h={28}/>
            </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:DS.s[4],marginBottom:DS.s[4]}}>
        <AreaChart data={rev} prev={revP} labels={mo} color={DS.colors.brand.blueLight} title="Receita Atribuída" sub="Atual vs anterior (milhares R$)" height={260}/>
      ...
      {/* ROW 3: Quick sections — Reuniões + Tarefas */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:DS.s[4],marginBottom:DS.s[4]}}>
        {/* Próximas Reuniões */}
        <div style={{background:DS.colors.gray.white,borderRadius:DS.r.md,border:`1px solid ${DS.colors.border}`,overflow:"hidden"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:`${DS.s[3]}px ${DS.s[4]}px`,borderBottom:`1px solid ${DS.colors.border}`}}>
            <div style={{display:"flex",alignItems:"center",gap:DS.s[2],fontSize:14,fontWeight:500,color:DS.colors.gray[700],fontFamily:DS.f.sans}}>
              <span style={{color:DS.colors.gray[400]}}>{Icons.bell(16)}</span> Próximas reuniões
            </div>
            <button style={{fontSize:12,fontWeight:500,color:DS.colors.brand.blueLight,background:"transparent",border:"none",cursor:"pointer",fontFamily:DS.f.sans}}>Ver todas</button>
          </div>
          {meetings.map((m,i)=><div key={i} style={{padding:`${DS.s[3]}px ${DS.s[4]}px`,borderBottom:i<meetings.length-1?`1px solid ${DS.colors.border}`:"none",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:13,fontWeight:500,color:DS.colors.gray[800],fontFamily:DS.f.sans}}>{m.t}</div>
              <div style={{fontSize:12,color:DS.colors.gray[400],fontFamily:DS.f.sans,marginTop:2}}>{m.dur}</div>
            </div>
            <span style={{fontSize:12,fontWeight:500,color:DS.colors.gray[500],fontFamily:DS.f.mono,flexShrink:0}}>{m.time}</span>
          </div>)}
        </div>
        ...
        <div style={{background:DS.colors.gray.white,borderRadius:DS.r.md,border:`1px solid ${DS.colors.border}`,overflow:"hidden"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:`${DS.s[3]}px ${DS.s[4]}px`,borderBottom:`1px solid ${DS.colors.border}`}}>
            <div style={{display:"flex",alignItems:"center",gap:DS.s[2],fontSize:14,fontWeight:500,color:DS.colors.gray[700],fontFamily:DS.f.sans}}>
              <span style={{color:DS.colors.gray[400]}}>{Icons.target(16)}</span> Tarefas pendentes
              <span style={{background:DS.colors.gray[100],color:DS.colors.gray[600],fontSize:11,fontWeight:600,padding:"1px 7px",borderRadius:DS.r.pill,fontFamily:DS.f.mono}}>4</span>
            </div>
          ...
          {tasks.map((t,i)=>{const p=pMap[t.p];return <div key={i} style={{padding:`${DS.s[3]}px ${DS.s[4]}px`,borderBottom:i<tasks.length-1?`1px solid ${DS.colors.border}`:"none",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:13,fontWeight:500,color:DS.colors.gray[800],fontFamily:DS.f.sans}}>{t.t}</div>
            <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 8px",borderRadius:DS.r.sm,background:p.bg,border:`1px solid ${p.b}`,fontSize:11,fontWeight:600,color:p.t,letterSpacing:"0.02em",flexShrink:0}}>
              <span style={{width:5,height:5,borderRadius:"50%",background:p.t}}/>{t.p}
            </span>
          </div>})}
        </div>
      </div>
