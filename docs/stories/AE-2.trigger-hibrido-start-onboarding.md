...
| 2026-05-29 | @dev | Adicionados 5 testes pedidos pelo reviewer: 3 cenarios de falha n8n (network error, AbortError/timeout, 5xx) + 2 race conditions (fresh+fresh -> 409, fresh+resume -> reused_batch). Bug descoberto e corrigido durante os testes: o service lancava `Error` simples com `statusCode=502` anexado, mas `errorResponse()` so honra `statusCode` em instancias de `AppError` — resultado era 500 em vez de 502. Substituido por `throw new AppError("n8n_dispatch_failed", 502, ...)`. Testes: 12/12 PASS. |
