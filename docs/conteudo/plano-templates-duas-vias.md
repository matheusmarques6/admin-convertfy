...
3. **Famílias visuais** — Editorial FEITA em 09/09 (ver CLAUDE.md): tokens em `lib/conteudo/familias.ts`, campos `gancho`/`anotacao`, destaque `**x**`, fontes Instrument Serif e Caveat, seletor na criação e no editor. Falta a família **Alternado** (CSS já em `design-system.md`), os presets de brand kit e os moldes Tendência/Previsão com `arco`. O card com sub-variantes (kpi_grid, print, conta à mão) e a seta da anotação continuam fora: dependem do segundo formato, que não chegou.
