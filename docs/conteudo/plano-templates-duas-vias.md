...
- **Arcos narrativos por tipo** (Tendência → Hook·Contexto·Mudança·Impacto·Ação; Tese contraintuitiva → Crença·Dados·Verdade·Novo modelo·Aplicação; Case → Resultado·Quem·Como·Princípio·Replicar; Previsão → Sinais·Padrão·Direção·Quem ganha·Ações): viram metadado `arco` dos moldes. Benchmark ≈ Case; Turbo ≈ Tese contraintuitiva; faltam **Tendência interpretada** e **Previsão** como moldes. *(set/2026: Benchmark, Turbo, Lista prática, MEC e Bastidor foram APOSENTADOS da prateleira — eram cinco sequências na mesma identidade azul. O mapeamento de arcos continua válido como plano; os moldes que o realizarem ainda serão escritos, cada um declarando a `familia` que pressupõe.)*
