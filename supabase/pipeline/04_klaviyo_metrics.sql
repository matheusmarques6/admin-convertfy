-- =====================================================
-- PIPELINE FILE 04: KLAVIYO METRICS SYNC SYSTEM
-- =====================================================
-- Creates: klaviyo_campaigns, campaign_metrics, campaign_metrics_history,
--          campaign_alerts, klaviyo_sync_config, klaviyo_sync_jobs tables
-- Creates: alert_type, alert_severity enums
-- Creates: check_campaign_metrics_alerts(), create_metrics_snapshot() functions
-- Creates: v_campaigns_with_metrics, v_campaign_rankings_revenue, v_unread_alerts views
-- Creates: All RLS policies (FIXED: is_admin() and can_access_store())
-- =====================================================
...
-- 2. Tabela de metricas atuais das campanhas
...
-- 3. Tabela de historico de metricas (snapshots para analise de tendencia)
...
-- 5. Tabela de configuracoes de sync por loja
CREATE TABLE IF NOT EXISTS klaviyo_sync_config (
  ...
  alert_open_rate_threshold DECIMAL(5,2) DEFAULT 10,
  alert_bounce_rate_threshold DECIMAL(5,2) DEFAULT 5,
  alert_unsubscribe_rate_threshold DECIMAL(5,2) DEFAULT 1,
  alert_performance_drop_threshold DECIMAL(5,2) DEFAULT 20,

  sync_campaigns_since DATE,
  sync_only_sent BOOLEAN DEFAULT TRUE,
...
-- Funcao para gerar alerta automaticamente quando metricas mudam significativamente
...
BEGIN
  ...
  -- Verifica taxa de abertura baixa
...
-- Funcao para criar snapshot automatico de metricas
...
-- View de campanhas com metricas (para listagem)
...
-- =====================================================
-- RLS POLICIES (FIXED: using is_admin() and can_access_store())
-- =====================================================
...
CREATE POLICY "klaviyo_campaigns_insert" ON klaviyo_campaigns
  FOR INSERT WITH CHECK (
    is_admin() OR can_access_store(store_id)
  );
...
-- Service role full access for all klaviyo tables
DROP POLICY IF EXISTS "Service role klaviyo_campaigns" ON klaviyo_campaigns;
CREATE POLICY "Service role klaviyo_campaigns" ON klaviyo_campaigns FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Service role campaign_metrics" ON campaign_metrics;
CREATE POLICY "Service role campaign_metrics" ON campaign_metrics FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Service role campaign_metrics_history" ON campaign_metrics_history;
CREATE POLICY "Service role campaign_metrics_history" ON campaign_metrics_history FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Service role campaign_alerts" ON campaign_alerts;
CREATE POLICY "Service role campaign_alerts" ON campaign_alerts FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Service role klaviyo_sync_config" ON klaviyo_sync_config;
CREATE POLICY "Service role klaviyo_sync_config" ON klaviyo_sync_config FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Service role klaviyo_sync_jobs" ON klaviyo_sync_jobs;
CREATE POLICY "Service role klaviyo_sync_jobs" ON klaviyo_sync_jobs FOR ALL TO service_role USING (true) WITH CHECK (true);

COMMIT;

-- =====================================================
-- 04_klaviyo_metrics.sql DONE
-- =====================================================
