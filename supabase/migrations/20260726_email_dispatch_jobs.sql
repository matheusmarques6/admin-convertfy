...
--
-- Problema: o botão "Gerar copies (n8n)" precisa rodar o Montador+Blueprint
-- de TODOS os emails ANTES de disparar pro n8n, mas isso (Opus, 60-180s/email,
-- 34 emails) não cabe no maxDuration de um request (504). Solução: enfileirar
-- um job; um cron (every minute, maxDuration 300) processa o Architect em
-- lotes e, quando TODOS os emails estão prontos (gerados ou esgotados →
-- fallback global), dispara pro n8n UMA vez.
--
