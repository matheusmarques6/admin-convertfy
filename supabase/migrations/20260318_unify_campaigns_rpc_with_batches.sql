...
--
-- 1. DROP old function signature (5 params) to allow new signature (7 params)
-- 2. CREATE new function with UNION ALL for campaign_batches
-- 3. Add p_limit / p_offset parameters
-- 4. total_count via COUNT(*) OVER()
-- 5. Status mapping for batches done in SQL (eliminates JS batchStatusMap)
--
-- PRESERVES: Story 45.4 NULL rate behavior (no COALESCE on rate fields)

-- =====================================================
-- 1. DROP OLD SIGNATURE
-- =====================================================
DROP FUNCTION IF EXISTS get_portal_campaigns_with_metrics(UUID[], DATE, DATE, TEXT, TEXT);

-- =====================================================
-- 2. CREATE NEW FUNCTION WITH BATCHES UNION + PAGINATION
-- =====================================================
...
RETURNS TABLE (
  ...
  color TEXT,
  -- metrics (COALESCE for counts, NULL for rates — Story 45.4)
  recipients INTEGER,
  delivered INTEGER,
  opened INTEGER,
  clicked INTEGER,
...
BEGIN
  ...
  -- Individual campaigns with metrics
  campaigns_cte AS (
    SELECT
      c.id,
      c.store_id,
  ...
  -- Campaign batches — unnest store_ids so each store gets a row
  batches_cte AS (
    SELECT
      cb.id,
      s_id                                                   AS store_id,
      ...
      CASE cb.status
        ...
        ELSE cb.status    -- 'scheduled', 'cancelled' pass through
      END                                                    AS status,
...
-- 3. PERMISSIONS
-- =====================================================
REVOKE ALL ON FUNCTION get_portal_campaigns_with_metrics(UUID[], DATE, DATE, TEXT, TEXT, INT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_portal_campaigns_with_metrics(UUID[], DATE, DATE, TEXT, TEXT, INT, INT) TO service_role;
